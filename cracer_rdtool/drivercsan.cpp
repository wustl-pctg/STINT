...
//                      uintptr_t addr, size_t mem_size, load_prop_t prop) {
// }

// void do_write(const csi_id_t store_id,
//                       uintptr_t addr, size_t mem_size, store_prop_t prop) {
// }
...
void __csan_load(csi_id_t load_id, const void *addr, int32_t size,
    ...
    //do_read(load_id, (uintptr_t)addr, size, prop);
	...
	while (size != 0) {
		int processed_size 
			= record_mem_helper(true, (uint64_t)CALLERPC, addr_t, size);
		addr_t += processed_size;
		size -= processed_size;
	}
...
void __csan_store(csi_id_t store_id, const void *addr, int32_t size,
    ...
    //do_write(store_id, (uintptr_t)addr, size, prop);
    if (!enable_checking) return;
	...
	while (size != 0) {
		int processed_size 
			= record_mem_helper(false, (uint64_t)CALLERPC, addr_t, size);
		addr_t += processed_size;
		size -= processed_size;
	}
...
void __csi_after_alloca(const csi_id_t alloca_id, const void *addr,  
	...
	//if (!enable_checking) return;
	size_t new_size = ALIGN_BY_NEXT_MAX_GRAIN_SIZE(size);  
	clear_shadow_memory((size_t)addr, (size_t)addr + new_size); 
}

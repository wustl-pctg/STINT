...
uint64_t num_check_race_update = 0;
int
                  ...
                  uint32_t mem_size)
{
  ...
  if( val == NULL ) {
    ...
    int max =  max_support_size(addr, mem_size);
	if (mem_size > max) {
		mem_size = max;
	}
  ...
  om_assert(val != NULL);
  //num_check_race_update++;
  return val->check_races_and_update(is_read, inst_addr, addr, mem_size, 

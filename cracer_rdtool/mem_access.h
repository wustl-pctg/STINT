...
//  pthread_spinlock_t writer_lock;
  ...
  int check_races_and_update_with_read(uint64_t inst_addr, uint64_t addr,
                              size_t mem_size, 
  ...
  int check_races_and_update_with_write(uint64_t inst_addr, uint64_t addr, 
                              size_t mem_size,
                              om_node *curr_estrand, om_node *curr_hstrand);
...
public:
  ...
  check_races_and_update(bool is_read, uint64_t inst_addr, uint64_t addr,
                         ...
                         om_node *curr_estrand, om_node *curr_hstrand) {
    if(is_read) {
      return check_races_and_update_with_read(inst_addr, addr, mem_size,
                                       curr_estrand, curr_hstrand);
    } else {
      return check_races_and_update_with_write(inst_addr, addr, mem_size,
          	                           curr_estrand, curr_hstrand);
    }
  }

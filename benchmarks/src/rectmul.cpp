 ...
 */
long long add_matrix(block *T, long ot, block *R, long orr, long x, long y) {
...
void init_matrix(block *R, long x, long y, long o, DTYPE v) {
  ...
  /** @todo doing this in parallel works in gcc, but not with
     clang. Is this a bug in clang? In either case I'm not going to
     chase it down, since everything works fine otherwise and
     initializing in parallel is not vital. It might also be fixed if
     we merge with upstream clang. */
  if (x>y) {
...
long long multiply_matrix(block *A, long oa, block *B, long ob, long x, 

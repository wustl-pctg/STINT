...
typedef struct MemAccess_t {
  ...
  // Return true if hstrand is either more left or deeper than curr_hstrand.
  inline bool leftmost_than(om_node *curr_hstrand) {
    om_assert(hstrand);
    om_assert(curr_hstrand);
    ...
    return !prec_in_hebrew;
  }
  
  // Return true if estrand is either more right or deeper than curr_estrand.
  inline bool rightmost_than(om_node *curr_estrand) {
    om_assert(estrand);
    om_assert(curr_estrand);

    /// @todo Is it worth it to join the batch relabel here?
    om_assert(self > -1);
    bool prec_in_english;

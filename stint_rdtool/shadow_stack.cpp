...
static uint64_t stack_high_watermark = (uint64_t)(-1);
...
extern "C" bool inParallel(MemAccess_t *reader1, MemAccess_t *reader2) {
	...
	num_of_sp_queries++;
#endif
	return (reader1->races_with(reader2->estrand, reader2->hstrand));
}

// return true if prevReader is more leftmost than currReader
extern "C" bool leftmost(MemAccess_t *prevReader, MemAccess_t *currReader) {
#ifdef SAMPLING
	num_of_sp_queries++;
#endif
	return (prevReader->leftmost_than(currReader->hstrand));
}

// return true if prevReader is more rightmost than currReader
extern "C" bool rightmost(MemAccess_t *prevReader, MemAccess_t *currReader) {
#ifdef SAMPLING
	num_of_sp_queries++;
#endif
	return (prevReader->rightmost_than(currReader->estrand));
}
...
extern "C" void do_task() {
    ...
    if (frames[w->self].empty()) {
		...
		if (enable_checking) {
			// checking is enabled, so we are 
			// ending a cont strand
			global_trace.end_of_strand();
		} else {
			enable_checking = true;
		}
	}
...
extern "C" void do_sync() {
    ...
    if (f->sync_english) {
		...
		if (frames[self].size() == 1) {
			// we synced into main strand
			// I believe it's safe to disable 
			// checking here
			enable_checking = false;
		}
    } else {
        //this is a function-ending sync
    }
    
    //frames[self].print_debug_info();
}

extern "C" void do_func_entry() {
	if (!enable_checking) return;
	//stack_high_watermark = (uint64_t) __builtin_frame_address(0); 
}

extern "C" void do_func_exit() {
	if (!enable_checking) return;
	stack_high_watermark = (uint64_t) __builtin_frame_address(1); 
	uint64_t stack_low_watermark = (uint64_t) __builtin_frame_address(0); 
	// clear local shadow mem
	assert((stack_high_watermark - stack_low_watermark) % 4 == 0);
  shadow_mem_read.bulk_clear_range(stack_low_watermark,stack_high_watermark-1);
  shadow_mem_write.bulk_clear_range(stack_low_watermark,stack_high_watermark-1);
	// clear treap
  Interval removeInterval = {stack_low_watermark, stack_high_watermark-1};
  read_treap = removeRange(removeInterval,read_treap);
  write_treap = removeRange(removeInterval,write_treap);
}

om_node* get_english() {
	return frames[self].head()->current_english;
}

om_node* get_hebrew() {
	return frames[self].head()->current_hebrew;
}
